...
export default function RebuildWatchHome() {
  ...
  useEffect(() => {
    async function syncESPN() {
      try {
        // Proxy ensures browser security doesn't block the data
  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-[#f5f5f5]">
      <p className="text-[#2f3e4e] font-black uppercase tracking-widest animate-pulse text-2xl italic">Rebuild Watch Syncing...</p>
    </div>
  );
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center gap-3">
            <button onClick={() => setStandings([...standings].sort(() => Math.random() - 0.5))} 
                    className="bg-[#2f3e4e] text-white px-12 py-3 rounded font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">
              Sim Lottery
            </button>
